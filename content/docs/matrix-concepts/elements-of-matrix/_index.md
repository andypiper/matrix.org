...
pieces of software that can bridge between Matrix and third-party platforms.
The Matrix Specification defines the interactions between them.
"""
+++
...
The Matrix ID of the users on the schema above, would be as in the diagram below.
...
are open source, so you can explore how they work. Please refer to their
...
[Ecosystem > SDKs](/ecosystem/sdks)). These will do significant parts of the
...
regular clients; instead of showing a UI to display what is happening, they
...
{{ figure(
    img="./bridge.svg",
    caption="A schema of a room bridged between matrix.org and Slack")
}}

To do this, the bridge needs to be able to create and impersonate users on Matrix,
and to control rooms as well. In order to limit the risks of abuse, bridges can
be limited to controlling a namespace.

To get a high-level view of concepts around bridges, and to see which platforms
Matrix can be bridged to, please head to [Ecosystem > Bridges](/ecosystem/bridges)
on this website. If you're interested in writing your own bridge, you will very likely
...
We have mentioned the [Matrix Specification](https://spec.matrix.org)
...
(homeservers, clients, appservices). Implementation details may vary for any
given feature, but Matrix aims for consistent behaviour and wants to avoid
